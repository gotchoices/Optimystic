...
# Arm: a commit whose cohort could not be resolved at all (GitHub #19, added 2026-09-14)

A fourth situation returns the same plain success, and a downstream user hit it on a four-node deployment. When every cohort lookup for a block throws, `ClusterCoordinator.getClusterForBlock` returns `{}`, so `getClusterPeerIds` returns `[]`. `CoordinatorRepo.isResponsibleForBlock` returns `true` on a throw ("assume responsible to avoid false rejections"). `commit` then sees `peerCount <= 1`, takes the solo branch, mints a single-signer proof, and returns `{ success: true }`. In the report, two phones each wrote a row, each got success, and each ended up holding only its own row with no error anywhere. The only trace is the `commit:solo-cohort` log line with `cohortSize: 0` or `soleIsSelf: false`. Beta.3's durability gate sits below that early return and never sees the case.

What this ticket must add for it:

- The durability class distinguishes a genuine cohort of one (`cohortSize 1`, sole peer is self) from a commit that reached no resolved cohort (`cohortSize 0`, or a sole peer that is not self). The second must not read as `local`. Something like `quorum: 'unrouted'`, or a separate routing flag, so a host can surface "saved on this device only; the network could not be reached" rather than "saved".
- The reporter's two-test reproduction in the issue uses only exported API and runs as-is on beta.3. Turn it into a spec: a commit whose every `findCluster` throws must not be indistinguishable from a healthy commit.
- Whether such a commit should be refused rather than reported is not this ticket's call. The responsibility-check half (fail closed on writes) is item 4 of `plan/self-in-cohort-only-when-nearest`. This ticket makes the outcome visible either way.
- The reporter asks for this before 1.0 final, since it changes a public result type.

# Evidence: a two-machine group commits alone after losing its partner (added 2026-09-15)

Phase 5 of `packages/db-p2p/test/small-deployment-lifecycle.integration.spec.ts` stops one machine of a two-machine group (default cluster size, `repairCorroborationClusterSize: 2`) and writes on the other. In 11 of 16 runs the write returned plain success once the survivor's routing view had dropped the absent peer: it went through the solo branch (`commit:solo-cohort`, `cohortSize: 1, soleIsSelf: true`), and until the partner returned the row existed on one machine only. That is situation 1 above, reached by a pair that lost a member rather than by a node that was always alone, and the caller could not tell. (In the other 5 runs the write was refused; whether it should be accepted at all belongs to `backlog/more-design/6.5-partition-healing`.)

Today that phase only logs the outcome. It checks that the returning machine can *read* the row, not that it holds a copy, because nothing pushes a missed write to a returning member. Once this ticket lands, phase 5 is the natural place to assert that an acknowledged away-write says `local`, and that the ledger drains it into the returning machine's own storage.
