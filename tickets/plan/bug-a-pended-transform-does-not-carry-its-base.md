...
## Promoted 2026-09-17 (night), with a release in the morning

The maintainer asked for tonight's work to be processed and the tree to be ready for a release in the morning. This ticket goes to plan so the design questions above get settled overnight. The release does not wait on it.

Guidance for the planner. The implement tickets you produce should separate the arms that need no wire or storage format change from those that do. The cheapest such arm is the read-driven promotion declining a record whose base it cannot establish, and letting block repair supply the version instead. Give the no-format-change arms the lower sequence numbers, and say in each ticket which arms change what a pend puts on the wire or what block storage keeps. The wire and storage change needs a compatibility answer for senders that supply no base. Settle that in the plan, not in implement.
