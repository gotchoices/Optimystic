...
Re-measured (`wc -l packages/db-p2p/src/repo/coordinator-repo.ts`): **2840 lines**, down from 2940 at HEAD immediately before that ticket (−100, almost all comment).

That ticket removed the absence memo again, because a one-machine view that later grew served a just-committed block as never created (GitHub issue #20). The fourteenth measurement's `settledAbsences` map, the `absenceSettled` verdict field and the memo's clearing rules are gone; read those notes as history. The class is back to four per-block `LruMap(1000)` fields (`responsibilityCache`, `lastSeenCommitMs`, `unsettledAheadClaims`, `stuckReservations`; `grep -c "new LruMap"` → 4). One new coupling for the extraction: at the solo-self exit, `lastSeenCommitMs` now also rate-limits the `cluster-fetch:solo-self-skip` log line for a block this node does not hold (the line and the stamp are both skipped while the stamp is younger than one window). A freshness collaborator should own that decision too, or give the line a stamp of its own.
