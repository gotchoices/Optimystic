...
## Fourteenth measurement (implement of `a-block-we-do-not-hold-is-consulted-on-every-read`)

Re-measured (`wc -l packages/db-p2p/src/repo/coordinator-repo.ts`): **2805 lines**, up from 2687 immediately before that ticket (+118, mostly comment) and from 2635 at the thirteenth measurement.

That ticket gave a block this node does not hold a freshness window of its own. It added a fifth per-block `LruMap(1000)` on the class (`settledAbsences`, joining `responsibilityCache`, `lastSeenCommitMs`, `unsettledAheadClaims` and `stuckReservations`; counted with `grep "new LruMap"`), recording when a consult last settled that the block's absence is the whole cohort's answer. It also added a third required field on the consult's verdict: `absenceSettled: boolean`, beside `absence` and `currency`. Two structural notes for the extraction. First, the map is deliberately not `lastSeenCommitMs`, because that map is also stamped for missing blocks at exits whose absence is *not* confirmed; reusing it is exactly the "hang the fact on the map that already exists" trap the third measurement described. Second, the field is the shape the thirteenth measurement asked for: every exit of `fetchBlockFromCluster` now *returns* whether it settled an absence rather than remembering to call a method, so an exit added later has to say which side it falls on. The held-block window (`markBlocksSeen` at four exits) is still in the old call-at-each-exit shape, and moving it into a returned field next to `absenceSettled` is now a small, obvious step. The clearing conditions a freshness collaborator would own grow by one more lifetime: the absence memo is cleared by a local pend or commit of the block, by seeing the block present, and by any later consult of it that does not settle.

Review of that ticket added a further piece of evidence. The memo's clearing rule, "a local pend or commit clears it", missed a writer: this node takes part in other coordinators' writes as a cohort member through `ClusterRepo.applyConsensusOperation`, which writes storage directly and never passes through `CoordinatorRepo`. With the memo armed on a multi-peer cohort, `fresh-node-ddl-multi` Scenario B served a just-committed block as absent in 5 of 20 runs, so review restricted settling to the solo exit (backlog `feat-a-cohort-member-remembers-a-settled-absence`). A freshness collaborator that owns invalidation needs to hear from every writer of this node's storage, not only from the coordinator's own pend and commit.

## Fifteenth measurement (implement of `drop-the-settled-absence-memo`)

Re-measured (`wc -l packages/db-p2p/src/repo/coordinator-repo.ts`): **2840 lines** after implement, down from 2940 at HEAD immediately before that ticket (−100, almost all comment); **2851** after its review moved the log rate-limit decision into a named helper.

That ticket removed the absence memo again, because a one-machine view that later grew served a just-committed block as never created (GitHub issue #20). The fourteenth measurement's `settledAbsences` map, the `absenceSettled` verdict field and the memo's clearing rules are gone; read those notes as history. The class is back to four per-block `LruMap(1000)` fields (`responsibilityCache`, `lastSeenCommitMs`, `unsettledAheadClaims`, `stuckReservations`; `grep -c "new LruMap"` → 4). One new coupling for the extraction: at the solo-self exit, `lastSeenCommitMs` now also rate-limits the `cluster-fetch:solo-self-skip` log line for a block this node does not hold (the line and the stamp are both skipped while the stamp is at most one window old; `soloAbsenceNamedThisWindow`). A freshness collaborator should own that decision too, or give the line a stamp of its own.

**Measured size (review of `commit-result-carries-durability-class`, 2026-09-15):** `wc -l packages/db-p2p/src/repo/coordinator-repo.ts` reports 2986 lines, of which that ticket added roughly 110 net (the solo short-circuit of `commit` became `commitSolo`, plus the three durability classifiers at the foot of the file). Extracting the freshness state this ticket describes is the natural first cut; the durability classifiers (`cohortDurability`, `cohortWriteDurability`, `pendCohortDurability`, `soloCohortDurability`) are a second self-contained slice if a split is undertaken.
