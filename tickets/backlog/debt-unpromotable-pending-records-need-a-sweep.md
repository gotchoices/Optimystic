...
Landed with `a-rival-pend-is-superseded-only-by-a-writer-that-built-on-it`. The rival rule (`isReservationAgainst`, `packages/db-p2p/src/storage/pending-claim.ts`) now reads the base the incoming pend declares for the block (`PendRequest.baseRevs`): a record is superseded only when that base is at or past the record's slot, and the requested-revision test above is the fallback for a pend that declares no base. That closes a lost update from four members up, but it means "the collection moved past the slot" no longer supersedes a record on its own — the block has to have moved past it too, as the writer read it.

So two populations the 2026-09-17 arm counted as covered come back to this ticket, both not decidable locally (a member cannot tell them from a live rival whose data-block commit is in flight, which is exactly the shape the new rule holds for):

- **An abandoned action's record on a block no later write touched.** Its data-block pend landed (that block's cohort), its log-tail pend did not (it lost the race on the tail, or the tail's cohort refused it), and its cancel never arrived. Before, the next write past its slot superseded it and that write's commit swept it. Now every later writer's base for the block is below the slot, so every member holding the record holds every writer, indefinitely.
- **The torn-sweep leftover from the 2026-09-04 arm, when its record carries no base** (written by an older sender). With a stored base it still clears: the next writer's read promotes it on the member that serves the read (`StorageRepo.get`, which promotes when the stored base is the member's latest), so the writer declares the record's slot as its base and the record is superseded. A base-less record cannot be promoted anywhere, and its change is on no member, so it holds like the case above. This one shrinks as older senders drain out of a fleet.

Both are named by the stuck-reservation lines (`coordinator-repo:stuck-reservation`, `cluster-member:stuck-reservation`) and recorded at the `NOTE:` on `isReservationAgainst`.
