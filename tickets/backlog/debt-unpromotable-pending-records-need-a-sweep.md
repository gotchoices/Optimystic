...
## Arm, 2026-09-17 — a neighbouring record that looks like this one and is NOT this one; and a correction to the arm above

Filed from `fix/1-a-write-gives-up-while-a-rival-still-holds-the-block`, now `implement/a-member-that-missed-a-commit-refuses-every-later-write`. Read this before designing the sweep, because the population it is sized against is smaller than the symptoms suggest.

**The correction.** The 2026-09-16 arm immediately above says the cost of a member lost mid-commit lasts "for as long as that member is away", and reports that once the member returned writes worked again, the first taking 2.5 s. Measured at three machines on the in-process mesh, with the member dropped one step earlier — during the **tail** commit rather than the sweep — that is not what happens. The record the returning member still holds outlives its own absence indefinitely: three consecutive writes from a healthy machine were each refused after 16.7–17.4 s with `SyncRetryExhaustedError` (`Pend blocks held: 1/3 member(s) hold an unresolved rival action (2/3 approvals)`), every machine reachable throughout, and they kept failing until the wedged member itself performed a read — after which the next write took 32 ms. Four runs, same outcome. The absence is the trigger; it is not the duration.

**Why that record is not yours.** It is **promotable**: its action is committed and durable on the rest of the cohort, and the member holding it is simply behind. The hard question this ticket exists for — how to tell an abandoned marker from an in-flight one without deleting a live reservation — does not arise, because the cohort's committed history is the authority and nothing has to be guessed or deleted. The cure is to finish the record, not to sweep it. That is the new ticket's job, at `ClusterMember.validatePendOperations`.

**What it means for sizing this ticket.** Two of the field reports that have been read as evidence for a node-side sweep — sereus's `control-write-degraded-cohort-member` run 1, and the `pending conflict: block(s) held by unresolved rival action(s) …` fingerprint generally — carry this shape too, and this shape is cured elsewhere. Whoever picks this ticket up should re-check how much of the observed population is left once a behind member stops vetoing on its own unreplicated records, rather than assuming all of it needs a sweep.

## Arm, 2026-09-17 — the cheap signal is now IN, and it covers more than the 2026-09-04 arm allowed for

Landed with `a-member-that-missed-a-commit-refuses-every-later-write`. Two things changed the population this ticket is sized against; read them before the design pass.

**A pending record now records the revision it was pended at** (`BlockMetadata.pendingRevs`, `packages/db-p2p/src/storage/struct.ts`), and both rival scans read a record as a claim on that slot: a record claiming a revision *below* the one an incoming pend requests is superseded and does not refuse (`isReservationAgainst`, `packages/db-p2p/src/storage/pending-claim.ts`). That is the "opportunistic check the next write to the block performs anyway" the body proposes, and it needs no timing guess. It covers the 2026-09-16 and 2026-09-17 arms above outright, and it covers the torn-sweep leftover from the 2026-09-04 arm as soon as the collection moves past the torn write's revision — which in that arm's own reproduction happens on the very next write, since the tail *did* commit. The 2026-09-04 arm's conclusion that the cheap signal "does not cover that case" was about the block-passed-the-revision test; the request-passed-the-revision test is the one that fires there.

**The block-passed-the-revision test is in too, as the sweep it was proposed as.** Every path that advances `latest` deletes the records whose claimed slot it has reached (`BlockStorage.sweepDeadClaims`, from `setLatest`, `recover` and `saveForwardRevision`). A record with no recorded revision is never swept.

**What is left for this ticket, stated exactly.** A record whose claimed slot the collection has NOT moved past — the writer's tail never committed, so every later writer still wants the same slot — and whose writer never comes back. That is the abandoned-marker population the body describes, and it is unchanged: not decidable locally, and still the case both stuck-reservation lines (`coordinator-repo:stuck-reservation`, and now `cluster-member:stuck-reservation` per member) name without acting on.

## Arm, 2026-09-18 — the base rule gives back part of what the revision rule covered

Landed with `a-rival-pend-is-superseded-only-by-a-writer-that-built-on-it`, narrowed at its review. The rival rule (`isReservationAgainst`, `packages/db-p2p/src/storage/pending-claim.ts`) can now read the base the incoming pend declares for the block (`PendRequest.baseRevs`): a record is superseded only when that base is at or past the record's slot. It reads it only at the promise vote, only in a cohort that can reach its promise bar without one member (`cohortCanMissAPend`: four members and up at the default 0.75 threshold), and only for a record that itself carries a stored base. Everywhere else — two- and three-member cohorts, the apply-time scan in `StorageRepo.pend`, and any base-less record (written by the release, or an inserted or deleted block) — the requested-revision test above still decides, exactly as the 2026-09-17 arm describes.

So one population the 2026-09-17 arm counted as covered comes back to this ticket, from four members up only:

- **An abandoned action's record on a block no later write touched.** Its data-block pend landed, its log-tail pend did not (it lost the race on the tail, or the tail's cohort refused it), and its writer crashed before cancelling. Before, the next write past its slot superseded it and that write's commit swept it. Now every later writer's base for the block is below the slot, so a member holding the record votes `held`. When the record stands on enough members to deny a super-majority (the writer crashed, so no member got the cancel), every later writer is held indefinitely. A record left on ONE member (its cancel was lost to that member alone) is outvoted, and that member's apply — revision rule — stores the newcomer's pend, whose commit then sweeps the record.

A member cannot tell that record from a live rival whose data-block commit is in flight, which is exactly the shape the base arm holds for. Named by the stuck-reservation lines (`coordinator-repo:stuck-reservation`, `cluster-member:stuck-reservation`) and recorded at the `NOTE:` on `isReservationAgainst`. (The review withdrew the second population the implement pass listed here — a base-less torn-sweep leftover — by keeping base-less records on the revision rule.)

## Arm, 2026-09-21 — records an older build wrote carry no revision, so neither rule ever retires them

Noted by the review of `debt-no-scenario-restarts-over-data-written-by-an-older-build` (static, not run). A pending record written by a build before `BlockMetadata.pendingRevs` existed — `1.0.0-beta.3` is the checked-in example in `packages/upgrade-check/fixtures/` — has no claimed revision. `isReservationAgainst` in `packages/db-p2p/src/storage/pending-claim.ts` returns `true` for a claim with no `rev`, and `BlockStorage.sweepDeadClaims` never sweeps one (the 2026-09-17 arm above). So after an upgrade, such a record whose writer never returns reserves its block for every later writer, however far the collection moves past it — where a record written by a current build stops reserving once a write requests a later revision. That is the documented conservative choice (an unknown slot is treated as the strongest claim), so it is a population for this ticket's sweep, not a separate defect. The upgrade-check suite commits its older build's in-flight record, so it does not exercise the abandoned case; a fixture whose writer abandons its pend without committing would, once this ticket has a sweep to test.
