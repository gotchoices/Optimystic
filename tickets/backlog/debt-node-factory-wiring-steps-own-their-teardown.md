...
### Update (2026-09-15): an eighth wrapper, for the relay-reservation supervisors

Found during review of `phones-never-re-reserve-after-their-relay-restarts`. The per-relay reservation supervisors (`superviseRelayReservations` in `packages/db-p2p/src/network/relay-reservation.ts`) are released by another hand-written `node.stop` wrapper, installed first inside the post-start span so the rollback `catch` reaches it. It is correctly placed and covered (`startup-rollback.spec.ts` asserts a relay that cannot be reserved releases the listener port), but it is one more instance of the pattern this ticket retires: nothing but its position in the file ties the wrapper to the supervisors it stops. Fold it into the collected-`stop` mechanism with the other seven.
