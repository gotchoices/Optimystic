...
## Arm: the new mesh case stores the log tail on every machine, not on a minority (review, 2026-09-16)

`half-landed-write-is-finished.spec.ts` tears at the transactor: it commits the log tail for real, through consensus, on all three nodes, and then *answers* failure. The production trigger is different in one respect that matters: the tail is held by **fewer than a majority** (that is why the durability gate refuses it), and the machines that lack it may also lack the block's previous revision. The writer's re-send then has to get the tail accepted by machines that never stored it, which is the path `backlog/bug-a-two-member-cohort-refuses-a-commit-both-members-hold` describes failing in a two-machine group. No mesh case drives the re-send against a minority-held tail. The `wrapRepo` seam described above (fail one member's commit of one block) is the layer that can express it.
