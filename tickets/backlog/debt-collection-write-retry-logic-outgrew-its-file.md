...
files:
  - packages/db-core/src/collection/collection.ts (`syncAttempts`, `syncInternal`, `completeOwnEntry`, `tornFromRefusal`, `consumeOwnEntry`, `beginInFlightAction`, `retainInFlightAttempt`, the `inFlightActionId` / `inFlightAttempt` fields)
