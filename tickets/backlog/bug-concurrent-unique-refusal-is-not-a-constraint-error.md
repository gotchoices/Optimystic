...
## Where the refusal now surfaces in legacy mode (added by the review of `legacy-multi-tree-commit-pends-everything-before-committing-anything`, 2026-09-17)

A legacy commit with several trees to push now goes through a per-commit coordinator (`commitBatchLegacy`), so its concurrent refusal reaches `commitTransaction`'s catch RAW — thrown by the coordinator's refresh between attempts — and is mapped there by `mapCommitRefusal`, exactly as session mode's is. The "already mapped" pass-through check now matters only for the per-tree fallback sweep (`sweepDirtyTreesLegacy`), which still maps inside the sweep. The fix's single seam is unchanged; `packages/quereus-plugin-optimystic/test/two-node-unique-value-race.spec.ts` matches the batch path's refusal on message only and should gain the type assertion with the two specs above.

# Downstream evidence, 2026-09-17 (sereus, against `fbf165ee`)

Sereus's two-real-node scenario `control-concurrent-unique-column-race.integration.ts` now gets a clean refusal in 6 of 6 rounds, across two processes of three rounds each. Before the legacy two-phase fix, both rows were stored in 6/6. **The loser's refusal can still surface as a plain `Error`, not a `ConstraintError`**, and per the review of `legacy-multi-tree-commit-pends-everything-before-committing-anything` so can primary-key refusals. Sereus has had to add a caveat about this to its schema guide. The data is now correct, so what remains is the error type, which is what this ticket is about. An application that maps constraint violations to user-facing messages will instead show a generic failure.
