...
`packages/db-p2p/src/cluster/cluster-repo.ts` is **2200 lines**
(`wc -l packages/db-p2p/src/cluster/cluster-repo.ts`, re-measured after
`commit-cert-digest-member-check` landed; it was 2067 after
`commit-and-cancel-records-omit-the-coordinating-block`, and 2034 after
