...
The read-path arm of this — an index *seek* returning a row that no longer holds the value — was a separate ticket, `index-seek-must-verify-its-entries`, and it has landed: the row an entry resolves to must still imply that entry. That re-check is now a shared exported predicate, `rowImpliesEntry` in `packages/quereus-plugin-optimystic/src/schema/index-manager.ts`, so whichever option below is chosen, deciding "is this entry a leftover?" is already written and does not need a second copy. Applying the same re-check to the uniqueness probe (`probeUniqueConstraint` in `packages/quereus-plugin-optimystic/src/optimystic-module.ts`) is **not sufficient**, and this is the part that makes the shape unsettled.
...
- **Remove the producers and stop there.** Three are known: concurrent same-row writes, which `refuse-concurrent-row-change-loser` closes; a delete issued from a handle whose index tree has never fetched the entry, whose delete is then logged but never applied on any node, which `blind-index-delete-is-logged-but-never-applied` closes (reproduced with no concurrency at all, so it is the easiest of the three for a user to hit); and a writer that was not maintaining the index updating a row, where a later re-attach adds the new entry and never purges the old one (see the `NOTE:` on `backfillIndexTrees`). The third survives both of those tickets, so this stays reachable — but if that producer is also closed, the tolerated-leftover machinery is dead weight. A fourth producer landed with `DROP INDEX` support (ticket `a-dropped-index-stays-in-the-plugins-catalog`): the drop leaves the index tree in storage, a later `CREATE INDEX` of the same name adopts it and re-stages every current row, and entries for rows deleted between the drop and the re-create survive. Closing that one needs a collection-delete primitive (nothing in the plugin deletes a collection today; `DROP TABLE` leaves its trees the same way) — see the `NOTE:` on `removeIndex` in `packages/quereus-plugin-optimystic/src/optimystic-module.ts`.
