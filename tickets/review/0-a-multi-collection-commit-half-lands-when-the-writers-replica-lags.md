...
## Review notes from the garden check-in (2026-09-18 ~01:00)

This is the only ticket blocking the morning release. Please check these specifically, on top of the usual review.

- **The assertions that changed are the heart of the review.** Nine existing tests were re-pointed. Decide for each whether it now asserts the new contract, or whether it asserts less than before. For example, `transaction.spec.ts` previously expected a cancel fault to propagate out of the commit; it now asserts `cancelCalls >= 1` and that the cancel fault does not replace the commit refusal. A contract change is fine if you can say why the new contract is right. A weakened assertion is not fine.
- **The `execute()` path keeps the old non-retrying partial fold** (the implementer's handoff flags this). Find out which production callers reach `execute()`, both the quereus plugin's session mode and sereus's paths. A multi-collection write on a lagging member through that path would still half-land. Either close it here, or say plainly why it cannot be reached by the sereus join and reconciler flows.
- **The retry is not gated on `conflict`.** A returned refusal that will never clear now costs the full retry budget (about 21 s) before it is reported. Confirm that no returned refusal is permanent in a way that makes the retry harmful, not just slow: re-applying a staged action onto a sibling that already committed, or re-pending over a record that was never cancelled. The new tests assert "logged exactly once" on both collections; check that those assertions actually cover the double-apply risk.
- **Acceptance downstream, read-only.** Run sereus's reproduction against the rebuilt `dist/`, three times, from `../sereus`. Only read that tree and run its tests there; do not edit, commit or build in it. Sereus's ticket runner is paused for this, so its `cadre-core` dist is stable. The command is `cd packages/integration-tests && yarn vitest run src/scenarios/strand-removal-cuts-network.integration.ts`. Before the fix, "leaves an open strand alone" failed 3 of 3 runs. Also run `src/scenarios/strand-membership-closed-strand-e2e.integration.ts` once. Report the counts. One known failure in the sereus suite is sereus's own (`control-offline-read-after-restart`, a stale block-index key expectation); it is not in these two files.
