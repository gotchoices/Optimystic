...
Every row assumes the block already has at least as many cohort-peer holders besides the reader as its
*must answer* column demands — those peers must answer *and agree*, which only a holder can do. See
...
| 4+ | either | 3+ | 2 | yes, survives one unreachable peer — **only for a block ≥2 peers hold; a block held by exactly one cohort peer never repairs, at any machine count with a floor of two, i.e. every row but the declared-two one (`reason: 'sole-holder'`)** |
