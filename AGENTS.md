...
`RUN_DCUTR_HOLEPUNCH=1` + `DCUTR_HOST` (needs a real non-private NIC address), and
`RUN_LONG_TESTS_CONTROL=1` (negative controls, which assert a *timeout* and so always burn their
...
`yarn.config.cjs` also requires every `peerDependencies` range to equal the same package's `devDependencies` range in that workspace, and each Quereus plugin's `engines.quereus` to equal its `@quereus/quereus` peer range. A peer range wider than the dev range advertises versions the package is never built or tested against — the Quereus plugins once declared `^4.3.0` while importing an export first shipped in Quereus 4.12.0. When bumping a dev range, move the peer range (and `engines.quereus`) with it.
