...
- `--assumed-cluster-size <number>`: How many peers you assert this deployment genuinely runs (positive integer; default 2). This is a *different* question from `--cluster-size`, which is how many copies to keep. It is read only by the membership admission gate, and only when the node has no confident network-size estimate — which is exactly what a partition causes. On that path the declared peer set must be at least 75% of this number, so a large deployment should set it to its real cohort size; otherwise a minority side of a partition can still assemble a write. Leaving it at the default is safe for a small mesh and is what lets a two- or three-node deployment transact unconfigured. Accepted by `interactive`, `service`, and `run`.
