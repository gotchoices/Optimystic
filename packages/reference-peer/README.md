...
- `--assumed-cluster-size <number>`: How many peers you assert this deployment genuinely runs (positive integer; default 2). This is a *different* question from `--cluster-size`, which is how many copies to keep. Two things read it: (1) the membership admission gate, only when the node has no confident network-size estimate — which is exactly what a partition causes; on that path the declared peer set must be at least 75% of this number, so a large deployment should set it to its real cohort size, otherwise a minority side of a partition can still assemble a write; (2) the read-repair/reconcile corroboration floor, unconditionally — a deployment that runs fewer peers than `--cluster-size` needs this set to its real peer count to be able to repair a damaged block at all. Leaving it at the default is safe for a small mesh and is what lets a two- or three-node deployment transact and self-repair unconfigured. Accepted by `interactive`, `service`, and `run`.
