...
describe('SqliteRawStorage', () => {
	...
	it('deletePendingTransaction removes it from the table and from listing', async () => {
		await storage.savePendingTransaction(blockId, actionId, makeTransform());
	...
	it('listBlockIds yields exactly the ids of blocks with metadata', async () => {
		...
		await storage.saveMetadata('b3' as BlockId, { ranges: [[1, 3]], latest: { rev: 3, actionId } });

		const out = new Set<string>();
		for await (const id of storage.listBlockIds()) out.add(id);
		expect(out).to.deep.equal(new Set(['b1', 'b2', 'b3']));
	});
