...
4. **Checks export routing.** `@optimystic/db-p2p` and `@optimystic/db-p2p/rn` must both resolve to `packages/db-p2p/dist/src/rn.js`, from every module that imports them: `entry.js`, and also the Quereus plugin and `@optimystic/db-p2p-storage-rn`, which import bare `@optimystic/db-p2p` themselves. If the `react-native` export condition were repointed or dropped, a React Native app would silently get the Node entry; this step fails instead.
