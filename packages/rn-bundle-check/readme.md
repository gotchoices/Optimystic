...
`entry.js` imports what [the db-p2p readme](../db-p2p/readme.md#react-native) tells a React Native app to import. `metro.config.cjs` aliases exactly the Node built-in module shims that readme's table lists, and `test/shim-table-parity.test.mjs` fails if the two drift apart. If the bundle only succeeds with an alias the table lacks, the readme has a gap: add the row in the same change. The config's one extra alias, a stub for the `react-native` package, exists only because this workspace does not install React Native itself.

On the Windows machine it was built on, a passing run took 12–15 seconds with a warm Metro cache (most of it `hermesc`) and about 30 seconds cold. Metro prints a handful of warnings about `multiformats` importing files its own `exports` map does not list. A host app sees the same warnings, and they never fail the check.
...
- **Sibling repositories are bundled from their working trees.** `p2p-fret` resolves through its portal link to `../Fret/packages/fret` and that checkout's own `dist/`, where a host app gets the published npm package. The freshness step does not cover it either, because it is not a direct dependency of this workspace.
