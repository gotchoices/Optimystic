...
describe('reactivity recover — RecoverRequestV1 / RecoverReplyV1 envelope', () => {
	...
	it('round-trips a checkpoint_window resume reply (the envelope carries the checkpoint chain + recentEntries)', () => {
		// The one inner resume variant whose extra fields (`checkpoints`, `recentEntries`) ride the envelope;
		...
		const reply: RecoverReplyV1 = { v: 1, kind: 'resume', resumeReply: { v: 1, result: 'checkpoint_window', checkpoints: [cp.summary()!], recentEntries: [note(17), note(18)], currentRevision: 18 } };
		expect(decodeRecoverReplyV1(encodeRecoverReplyV1(reply))).to.deep.equal(reply);
	});
