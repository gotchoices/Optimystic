...
async function createNode(port: number, bootstrapNodes: string[], networkName: string): Promise<TestNode> {
	...
	// Register virtual tables and functions
	if (plugin.vtables) {
...
async function runTestScenario(nodes: TestNode[]) {
	...
	for (let i = 0; i < nodes.length; i++) {
		...
		if (results.length > 0) {
			console.log(`      ${results.map((r: any) => r.name).join(', ')}`);
		}
	}
	...
	for (let i = 0; i < nodes.length; i++) {
		...
		const result = await nodes[i]!.db.prepare('SELECT TransactionId() as txn_id').get();
		await nodes[i]!.db.exec('COMMIT');
		console.log(`   ✓ Node ${i + 1}: ${result?.txn_id?.slice(0, 16)}...`);
	}
