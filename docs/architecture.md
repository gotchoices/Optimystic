...
The cohort-topic substrate itself is now **implemented** — its protocol-agnostic logic (walk, registration, willingness, promotion, gossip, threshold sigs, membership, traffic, anti-flood/anti-DoS, the participant-facing `CohortTopicService`, and the cohort-side `CohortMemberEngine`) lives in `packages/db-core/src/cohort-topic/`, with the FRET/libp2p host in `packages/db-p2p/src/cohort-topic/`; mock-tier end-to-end coverage now lands an in-process multi-node cohort (real Ed25519 keys, mock transport routing the five cohort protocols + FRET `routeAct`/`assembleCohort`) in `packages/db-p2p/test/cohort-topic/live-tier.spec.ts` — a real cohort registers, threshold-signs, gossips, and promotes end-to-end; the real-libp2p (socket) e2e now lands the substrate over real TCP (`substrate-real-libp2p.integration.spec.ts` — real FRET assembly, `/sign` threshold-cert + `/membership` verify, `/cohort-gossip` replication), with the two application-transport seams (reactivity notification delivery, matchmaking query RPC) still unwired and tracked. The two applications below are still being built out on the substrate: **matchmaking is now implemented at the unit/typecheck level across its full surface** (`packages/db-core/src/matchmaking/`, `packages/db-p2p/src/matchmaking/`) — wire codecs, the stable topic anchor, provider/seeker registration, query/filter evaluation, the hang-out decision engine, the hot-topic multi-cohort sweep + threshold-signed aggregate counts, the seeker-side adversarial-traffic bounds (with reputation cross-check hooks), and the cohesive public module (`MatchmakingProviderSession` / `MatchmakingSeekerSession` + the voting `QuorumDiscovery` binding) — with only its mock-tier e2e still pending; reactivity remains **specified / design-only**. A **simulator phase validated the design's quantitative claims before the core protocols landed**; see the specs ([cohort-topic.md](cohort-topic.md), [reactivity.md](reactivity.md), [matchmaking.md](matchmaking.md)) and the per-subsystem status table below.
...
The networked cohort-topic substrate, reactivity, and matchmaking are validated and rolled out in milestones. The **Simulator validation** column is flipped to `done` by `fold-simulator-findings-into-design-docs` — the design simulator (`packages/substrate-simulator`) has measured every quantitative claim in the three specs and folded the results back (depth law confirmed at N ∈ {100 … 1M}, `coord_d` collision rate ~0, promotion overshoot/convergence characterized, replay/checkpoint coverage and the adaptive-`W` finding settled, hang-out math and `contention_factor_cap` justified). `fold-envelope-into-design-docs` then extends that column with the **operating envelope**: the validity-envelope finder (`boundary.ts` + the `boundary-*` axis modules) measured, per claim, the edge at which it flips pass→fail along a worsening-condition axis and the margin from that edge to the design's operating point, now written into each spec as an **Operating envelope** subsection (e.g. the depth law holds to arrivals-per-round `R < 666` and prefix-skew `s < 0.042`; revision-continuity to `cps < 72.5` on the layered `W + W_checkpoint` window; plus one honest **negative**-margin caveat — matchmaking hang-out-fairness misfires at seeker:provider ratio `ρ < 2.5`, *below* the `cap − 1 = 3` saturation point, which is the principled signal that the deferred exact-`Σ wantCount` contention refinement is warranted). The cohort-topic **Mock-tier e2e** column is flipped to `done` by `cohort-topic-live-tier-e2e` (`live-tier.spec.ts` — an in-process multi-node cohort over real Ed25519 keys + a mock transport) and completed at scale by `cohort-topic-e2e-mock-tier`: the extracted, reusable `src/testing/cohort-topic-mesh-harness.ts` drives production-shaped cohorts (`k = 16`, `minSigs = 14`) in 48–64-node rings through registration/TTL-renewal, per-tier willingness gating, promotion + sticky/root-never-demote hysteresis, crash failover, gossip replication, and the §Anti-flood / §Anti-DoS disciplines (`cohort-topic-scale-lifecycle.spec.ts`, `cohort-topic-scale-antiflood.spec.ts`), driven by an explicit virtual clock (no wall-clock sleeps). Doc expectations the single-tier-0 milestone cannot yet satisfy are tagged `it.skip([… DOC EXPECTATION NOT YET IMPLEMENTED …])` against their parking ticket rather than omitted (multi-tier depth/follow-on, tier-(d>0) demotion notice, rotation handoff, and the participant-coord prefix-collision fan). The matchmaking **Mock-tier e2e** column is flipped to `done` by `matchmaking-e2e-mock-tier`: `src/testing/matchmaking-mesh-harness.ts` (layered on the cohort-topic mesh harness) drives the real provider/seeker managers, the real cohort `QueryV1` handler, and the real seeker walk client over the in-process mesh — provider register → cohort store → query round-trip + `registrationSig` re-validation, capacity self-throttling / TTL-withdrawal, the cold/sparse-walk-to-root and borderline hang-out regimes (modeled traffic over the real walk), adversarial over-report bounds, and the multi-cohort sweep binding (`runMultiCohortSweep` over the real `buildAggregateCount` + real shard records — the port-binding the sweep ticket deferred). Doc expectations needing a *serving* promoted tier-`d ≥ 1` cohort (hot-deep-tier, tree-promotion depth tracking) or membership-rotation primary handoff are tagged `it.skip([… DOC EXPECTATION NOT YET IMPLEMENTED …])` against their cohort-topic parking tickets. The reactivity **Mock-tier e2e** column is flipped to `done` by `reactivity-e2e-mock-tier`: `src/testing/reactivity-mesh-harness.ts` (layered on the cohort-topic mesh harness) drives the **real** reactivity hot path over the in-process mesh — commits flow through the real local-change-notifier bridge → real origination (reusing the commit cert's threshold signature unchanged) → real forwarder receive path (verify → dedupe → replay ring + rolling checkpoint) → the real `ReactivitySubscriptionManager` delivery, **verified end-to-end against the tail cohort's `MembershipCertV1` with real Ed25519 collected-multisig crypto** (no pass-crypto stub). The suites cover cold-to-hot growth + contiguous/deduped/verified delivery, the four mobile-resume variants over the stacked `W`/`W_checkpoint` windows (backfill/checkpoint/out-of-window/tail-rotated), tail-rotation continuity (pre-announce → handoff checkpoint → gap-free revision stream, jittered re-registration within `cap_promote_fast`, drain gate), partition healing (no loss / no double-delivery, sliding dedupe window), and slow-subscriber drop-without-stall + Edge-never-forwards (`reactivity/mesh-{cold-to-hot,mobile-resume,tail-rotation,partition-healing,slow-subscriber}.spec.ts`). It **supersedes the intent** of the `backlog/optimystic-network-reactive-watch-integration-test` stub (a single networked reactive-watch test), generalizing it to the full reactivity surface at scale. Doc expectations needing a *serving* multi-tier forwarder fan-out (deep-tree depth tracking) or the at-scale rotation burst / coverage-window magnitudes (the design simulator's quantitative claims) are tagged `[unimplemented:mock-tier]` in the suites rather than faked. The **Real-libp2p e2e** column is flipped by `substrate-e2e-real-libp2p-tier` (`packages/db-p2p/test/substrate-real-libp2p.integration.spec.ts`, env-gated on `OPTIMYSTIC_INTEGRATION=1` / `RUN_LONG_TESTS=1`, 3–16 production `createLibp2pNode({ cohortTopic: { enabled: true } })` nodes over real TCP, validated at N ∈ {3,4,8,16}): real FRET two-sided stabilization assembles the whole-mesh tier-0 cohort with one agreed coord/epoch on every node (the piece the mock mesh stubs); a routed primary collects a genuine `(N−1)`-of-`N` threshold `MembershipCertV1` over the real `/sign` RPC, serves it over `/membership`, and a remote participant's verifier verifies it end-to-end (including the one-fetch-and-retry on a stale cached cert); a record replicates into a sibling store over real `/cohort-gossip`; the reactivity origination membership gate agrees with the real FRET cohort and the production bridge is installed; a matchmaking provider record lands in + replicates across a real cohort. **Two application-transport seams remain unwired in production and are tagged `it.skip` (not faked) with their tracking tickets:** reactivity notification *socket delivery* (no emit transport / delivery protocol — `12.5-reactivity-tail-rotation-transport`) and the matchmaking *seeker `QueryV1` RPC* (no query handler registered on a real node). The cohort-topic substrate itself is fully exercised over real sockets; the full FRET-routed participant `service.register` walk and multi-tier promotion stay mock-tier-deterministic (tagged) to avoid small-N routing flakiness.

| Subsystem | Simulator validation | Mock-tier e2e | Real-libp2p e2e |
|-----------|----------------------|---------------|-----------------|
| cohort-topic substrate | done — claims + operating envelope (depth-law `R*`/`s*`, churn/walk/partition margins) | done (`live-tier.spec.ts` + at-scale `cohort-topic-scale-{lifecycle,antiflood}.spec.ts` over `cohort-topic-mesh-harness.ts`) | done (`substrate-real-libp2p.integration.spec.ts` — real FRET assembly + coord derivation, `/sign` threshold-cert + `/membership` verify incl. stale-refetch, `/cohort-gossip` replication; multi-tier/promotion + full FRET-routed walk tagged `it.skip`) |
| reactivity | done — claims + operating envelope (continuity `cps*` on the layered window, drain-ratio margin) | done (`reactivity/mesh-{cold-to-hot,mobile-resume,tail-rotation,partition-healing,slow-subscriber}.spec.ts` over `reactivity-mesh-harness.ts`) | substrate done; **delivery deferred** (`substrate-real-libp2p.integration.spec.ts` — origination membership gate over real FRET + bridge wiring + real cert-verify hop; notification *socket delivery* is unwired in production → `it.skip`, tracked by `12.5-reactivity-tail-rotation-transport` / backlog `optimystic-network-reactive-watch-integration-test`) |
| matchmaking | done — claims + operating envelope (bounded-harm `f*`; **negative**-margin hang-out-fairness `ρ*`) | done (`matchmaking/mesh-{lifecycle,walk,sweep}.spec.ts` over `matchmaking-mesh-harness.ts`) | substrate done; **query deferred** (`substrate-real-libp2p.integration.spec.ts` — provider register + `/cohort-gossip` replication over a real cohort; the seeker `QueryV1` RPC handler is unwired in production → hang-out-walk-converges `it.skip`) |
...
* **Block** — versioned storage unit with a stable base64url `BlockId` (random for data blocks, name-derived for collection headers); its revisions are immutable but not content-addressed (each maps to an `ActionId`, itself typically random — see Transaction ID for the one content-addressed identifier), and the `BlockId` is not a content hash either.
