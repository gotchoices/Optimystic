...
**Holder evidence.** Every block that lands here enters the owned-block set with no growth memory, so on its own the next check would report it `gained` (a pull over `/sync`) and its whole cohort `grown` (a push over `/block-transfer`), sending the block straight back to the peers it came from. So a block this node holds seeds the memory through `RebalanceMonitor.recordBlockHolders({ blockIds, holders, unconfirmed? })`, fed by the `BlockHoldersSink` (`onBlockHolders`) that `libp2p-node-base` routes to the monitor. The premise every producer asserts is the same, and both halves matter: *these peers are evidenced to hold this block, and this node holds it too, so it needs no pull.*
