...
- **Move-in** (`R → R-1`): Phase A only — sheds nothing, advertises the inner ring. Nothing fetches the gained half: it arrives via the existing holders' own cohort-growth push (§ RebalanceMonitor above), with a read repairing any gap on first access.
