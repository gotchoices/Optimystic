...
interleave with a mid-flight commit; outside a transaction each scan resolves its own instance
with its own tracker, so there is no shared collection state to serialize). Writes still
