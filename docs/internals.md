...
**Commit evidence.** Every block a commit touches enters the owned-block set with no growth memory, so on its own the next check would report it `gained` (a pull over `/sync`) and its whole cohort `grown` (a push over `/block-transfer`), sending the block straight back to the members that stored it as part of the commit. So a commit this node holds seeds the memory through `RebalanceMonitor.recordCommittedHolders({ blockIds, holders, unconfirmed? })`, fed by the `CommittedHoldersSink` (`onCommittedHolders`) wired to both consensus sites. The member reports after a durable consensus apply, naming the record's approving commit signers. The coordinator reports when it acknowledges a cohort commit it holds, naming the confirmed members that also signed, and names the members the durability class left unconfirmed. Evidence waits for the next check, which folds `holders` into `cohortPeers`, removes `unconfirmed` from it, and does not report the block `gained`. Later evidence about a peer overrides earlier evidence, so the coordinator's report corrects the signer list its own member sent moments earlier. A solo commit reports nothing, so the founder case still reports the whole cohort once peers appear.
