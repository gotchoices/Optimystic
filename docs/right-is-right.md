...
**Invalidation certificate.** A member applies an `InvalidationEntry` only if its `resolution` is a valid certificate — `outcome === 'challenger-wins'` and the `agree-with-challenger` votes meet the 2/3 decisive-vote threshold, each Ed25519 signature verified against its arbitrator's embedded key, each vote bound to the specific reversed transaction and counted at most once per arbitrator. This is the reversal analogue of the commit certificate: the *intent* is that a compromised peer can withhold an invalidation but **cannot forge** one. Application is idempotent, keyed on `(invalidatedActionId, disputeId)`.

> **Unforgeability — target binding (#2) and per-arbitrator dedup (#3) now hold; arbitrator-set legitimacy (#1) still open.** Votes are now signed over a *target-bound v2 payload* `v2:${disputeId}:${vote}:${computedHash}:${targetHash}`, where `targetHash = hashString(messageHash | invalidatedActionId | sortedBlockIds)`. `verifyInvalidationCertificate(proof, target)` recomputes that hash from the proof's `messageHash` plus the **apply-path target** (the action/blocks the request actually reverts), so a genuine proof for transaction X can no longer be replayed against an unrelated transaction Y — every signature fails. The count loop dedups by `arbitratorPeerId` (one vote replicated N× counts once) and drops an equivocating arbitrator from both sides, so a single vote can no longer manufacture a super-majority. v1/unversioned votes are rejected, not accepted-by-default. **Still missing:** the votes are not yet bound to the *legitimately-selected arbitrator set*, so a peer that can mint Ed25519 keypairs can still present a synthetic cohort. That last binding is `tickets/implement/7.8-invalidation-cert-arbitrator-set-binding.md` and **must** land before the live composition-root wiring. No durable reversal is originated or applied end-to-end yet, so this is not a live exploit today.
