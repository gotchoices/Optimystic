...
> **Status (Theorem 2):** The core partition-safety argument is largely **current**: membership binding (§2) and the member-side membership admission gate both landed (prereqs `bind-cluster-membership-into-signed-record`, `cluster-membership-admission-gate`). Two caveats: (1) **Compound margin, not the simple count** — the code default matches the proof's ≥75% assumption exactly: `superMajorityThreshold` resolves to `DEFAULT_SUPER_MAJORITY_THRESHOLD` = 0.75 (`packages/db-core/src/cluster/structs.ts`) via `resolveClusterPolicy` (`packages/db-p2p/src/cluster/cluster-policy.ts`) whenever no explicit override is configured. But the proof's simple counting argument (⌈0.75K⌉ + ⌈0.75K⌉ ≤ K) assumes both sides vote on the *full* `K`; because the admission gate admits a set as small as `membershipAdmissionFraction · K`, the condition that actually has to hold is `2 · membershipAdmissionFraction · superMajorityThreshold > 1`. At the shared default (`membershipAdmissionFraction` also defaults to 0.75, `packages/db-p2p/src/cluster/cluster-repo.ts`) that product is `2 · 0.75 · 0.75 = 1.125` — comfortably true, and both the confident and the low-confidence admission path sit on it. Lowering either default (or raising `clusterSizeTolerance` enough to widen what counts as the same cluster) shrinks that margin and can break the contradiction, so re-derive this product before changing either. (2) **Escalation clause** — if a minority partition were somehow to form a disputed super-majority despite the gate, the only recourse would be dispute escalation to recruit a wider honest majority (same mechanism as Theorem 1 Case 3). That escalation path is target / not built; the gate is the primary current defense. (3) **The gate is armed on PEND only, today** — a member re-derives its own view `E` from the coordinating block id carried on the record, and only `CoordinatorRepo.pend` puts that field on its message. On the `commit` and `cancel` paths the field is absent, so no `E` is derived, the confident predicates never run, and the record is judged against the low-confidence `assumedClusterSize` floor instead. Partition safety still rests on pend — a minority side that cannot pend never reaches commit — but the commit step itself is currently guarded only by that weaker floor, and a cohort admitted at pend can be refused at commit (a write stranded pended-but-uncommitted). Tracked by ticket `commit-and-cancel-records-omit-the-coordinating-block`; `packages/db-p2p/test/mesh-partition-admission.spec.ts` pins the current behavior.
