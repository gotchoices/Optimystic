...
When conflicting transactions race, `resolveRace()` deterministically selects exactly one winner. The order is: **(1) more approvals, (2) equal approval counts → higher aged priority, (3) still tied → higher message hash.** The winner commits. The loser is rejected and may retry.
