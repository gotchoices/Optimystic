...
*Commit signatures are not yet a trust anchor on this path.* A commit does produce a super-majority threshold signature over the committed action, and in principle a recovering node could verify it against known peer identities and accept the block regardless of who served it. **That is not what happens today**, though the artifact now exists and now travels. The durable `BlockCommitProof` (`packages/db-p2p/src/cluster/commit-proof.ts`, see [internals.md](internals.md#durable-commit-proof-blockcommitproof)) is persisted by the cohort members that committed the revision, `BlockArchive` carries it in the revision entry it certifies, and both repair wires deliver it: the archive a peer serves a repair fetch with, and the latest-revision claim projected out of that archive (`CertifiedActionRev`, `RevClaim.proof`). What is still missing is the *deciding*: no consumer verifies a proof or weighs it, so corroboration still counts distinct peers exactly as it did before, and recovery integrity still rests entirely on the two mechanisms above. Closing that (and with it the Sybil exposure that majority-counting alone cannot address) is tracked as `accept-certified-claims-in-repair`; a verifying consumer must also corroborate the cohort itself, since a passing proof says the listed cohort agreed, not that it is the right cohort for the block.
