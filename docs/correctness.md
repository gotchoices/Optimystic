...
**Pend refusal reporting.** Before any of this, a member whose storage *reserves* the blocks for a different unresolved action — a pending record claiming the requested revision or a later one, never one the collection has already moved past (see Theorem 9) — refuses on the promise round rather than at apply (`ClusterMember.validatePendOperations`), with the `held` vote Theorem 1 Case 2 describes. That refusal is signed evidence in its own right, so the coordinator answers the writer a retryable conflict on the strength of the votes alone; its own local re-read only *enriches* that answer with the rival action ids (`StaleFailure.pending`, and the input `CoordinatorRepo.noteStuckReservation` needs to name a block wedged behind a reservation that will never clear). Making the re-read a gate was wrong for a reason worth keeping written down: under delivery latency the refusing member is routinely ahead of the coordinator, so the corroboration missed and a transient refusal escaped as a permanent verdict.
...
A rival that has already *won* its pend is refused from storage instead, with the same separation and the same outcome: a `held` vote naming the holder's action id, surfaced as `BlocksHeldError` → `StaleFailure` with `conflict: true` (see Theorem 1 Case 2 for why the two kinds cannot share a field). Both are terminal for the record they answer and retryable for the writer — and the retry can actually win, because a member only casts `held` for a record that its holder is still going to remove. A pending record reserves the block for the *slot* it was pended at (the revision kept beside it in `BlockMetadata.pendingRevs`), and `isReservationAgainst` in `packages/db-p2p/src/storage/pending-claim.ts` is the one rule both refusal sites apply: a record claiming the requested revision or a later one reserves; one claiming an earlier revision has been superseded by the collection moving on — its action either committed at that slot on the rest of the cohort (this member missed the commit) or lost the slot — and does not. Refusing on a superseded record was a liveness defect rather than a theoretical one: a member that promised a write and missed its commit vetoed every later write to the block, from every writer, until something happened to read the block through it (ticket `a-member-that-missed-a-commit-refuses-every-later-write`, measured at three machines as three consecutive writes each exhausting their full retry budget). Approving over it is safe because the incoming writer built on that commit's outcome: its read context names every action the log has committed, and `StorageRepo.get` promotes a held record for a named action before serving the block, with the block floor (`BlockGets.floors`) as the second line. That covers every member that holds the record; the shape it does not cover — from four members up, a member that never received the rival's pend answering a floor-less handle while the rival's non-tail commit is still in flight — is recorded at `isReservationAgainst` and owned by ticket `a-rival-pend-is-superseded-only-by-a-writer-that-built-on-it`. The member comes current when the approved pend's own commit applies (`StorageRepo.internalCommit`, or the behind-reconcile its fork guard triggers), at which point the superseded record is provably dead and swept (`BlockStorage.setLatest` and `saveForwardRevision`).
