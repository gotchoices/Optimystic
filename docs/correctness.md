...
**Pend refusal reporting.** Before any of this, a member that *already* holds the blocks under a different unresolved action refuses on the promise round rather than at apply (`ClusterMember.validatePendOperations`), with the `held` vote Theorem 1 Case 2 describes. That refusal is signed evidence in its own right, so the coordinator answers the writer a retryable conflict on the strength of the votes alone; its own local re-read only *enriches* that answer with the rival action ids (`StaleFailure.pending`, and the input `CoordinatorRepo.noteStuckReservation` needs to name a block wedged behind a reservation that will never clear). Making the re-read a gate was wrong for a reason worth keeping written down: under delivery latency the refusing member is routinely ahead of the coordinator, so the corroboration missed and a transient refusal escaped as a permanent verdict.
