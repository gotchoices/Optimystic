...
| `under-replication-ledger` | The durable record of blocks acknowledged below full replication: `record:skip-lower-rev` (a slower, older commit did not overwrite a newer shortfall), `settle:keep-higher-rev`, `satisfy:keep-higher-rev` (a copy of an older revision landed after a newer shortfall was recorded), `name:keep-other-rev` (the drain resolved a cohort for an entry that was replaced mid-push), `evict:over-cap` when the entry cap evicts the oldest entry, and on `:error` `evict:failed` plus `read:unparseable` / `read:malformed` for a stored entry read as absent |
| `under-replication-drain` | Sending the copies the ledger says are owed: `pass:done` with per-pass counts (`solo` entries whose cohort is this node alone, `waiting` entries with nobody reachable, `pushed`, `cleared`, `dropped`, `abandoned`), `pass:skip` with `reason=partition`, `push:ok`, `push:rejected` (the receiver refused — "no retained proof — cannot place" when this node holds no proof for the revision) as distinct from `push:unreachable`, `entry:cleared` when the last missing member confirmed and the full-replication event fired, `entry:dropped` for a block gone from local storage, `peer:abandoned` after the give-up bound, `cohort:unresolved`, and `throttled` for a trigger deferred to the end of the minimum interval |
...
Counted by `ageMs` on the reporter's next capture, the result was 290 triggers. Eight had no stamp, one first touch per block. The other 282 were above the 10 s window, and **none** were at or below it. The reporter withdrew the claim. Their gap statistic had been counted over `cluster-fetch:solo-self-skip` lines, which on that release fired for every consult on a node that is its block's whole cohort, including consults about a block the node does not hold. None of the 254 skips for the hot block had a trigger line before them. They were many separate reads of one frequently read block, each correctly consulting, not one read re-entering.

Two lessons carry over to any capture. Classify each trigger by `ageMs`, never by the gap since the previous one. And a `cluster-fetch:solo-self-skip` with no matching `read-repair-triggered` is a consult about a block this node does not hold, not a re-entry of the window. On current releases that second kind is logged at most once per block per `readRepairWindowMs`, although every read of a block the node does not hold still consults, so counting those lines no longer counts consults. For a block the node holds, the line still appears once per consult.
...
|---|---|---|
| `silent` above `0` | The pass did not see the whole cohort, and a silent peer may be the missing holder. | Wait, or fix reachability — **or raise the deadline**, if the link is simply slower than it. A peer counts as silent when it does not answer within `clusterPolicy.cohortQueryTimeoutMs` (default 1000 ms), and every one of these requests is a fresh stream: dial or reuse, protocol select, send, receive. A relayed link whose round trip is near 1.8 s can never finish one inside a second, so healthy peers answering everything else read as silent on every read — and in a two-member cohort one late answer is the whole quorum. Raise the field to comfortably exceed the link's real round trip; see [transactions.md §Read Consistency and Staleness](transactions.md#read-consistency-and-staleness). |
...
- `retry` `undefined` or missing — no commit retry is pending. The coordinator drops the transaction's entry about 100 ms later and logs `cluster-tx:transaction-remove` with `reason: 'complete'`.
- `retry` present, as `{ attempt, pending }` — some cohort members missed the commit broadcast, and a background retry is still sending it to the peer ids in `pending`. **No `transaction-remove` follows yet.** It follows whichever way the retry ends: `cluster-tx:retry-finished`, once every pending peer has the commit (`reason: 'retry-finished'`), or `cluster-tx:retry-abort`, when the retry budget runs out first (`commitBroadcastRetryMaxAttempts`, default 5) with peers still missing (`reason: 'retry-abandoned'`).
