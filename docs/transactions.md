...
- **The same cap applies to the commit-path reconcile, on both of its gates.** `reconcile-block.ts` first picks the revision (as above) and then requires the cohort to agree on the block *content* at that revision. Both gates take the same capacity, so a two-node deployment that has declared `clusterSize: 2` can move bytes as well as pointers; leave `clusterSize` at its default of 10 in a two-node mesh and the heal declines with `reconcile:no-rev-quorum` or `reconcile:no-content-quorum`. Note what the content gate is and is not: block ids are random 256-bit strings, not content hashes, so a peer's bytes are checked against *other peers' bytes*, never against the id you asked for. With one other peer there is nothing to check against, and its content is accepted on the same trust its revision claim already receives.
