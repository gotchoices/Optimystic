...
**A block this node does not hold is windowed too — but only when this node is its whole cohort.** A read of a block missing locally consults the cohort; that consult is what lets the coordinator tell "nobody has this" from "I don't have this". When `findCluster` returns only this node, the consult *settles* the absence — nobody else could hold the block, and any commit of it lands in this node's storage before the writer hears success — so the coordinator remembers it for one `readRepairWindowMs`, and reads inside the window serve the authoritative absent without consulting again. Repeatedly probing a record that has not been created yet on a one-machine deployment (the new-collection probe, or an empty collection read before every membership check — GitHub issue #8) therefore costs one consult per window per block instead of one per read. A multi-peer cohort's unanimous "holds nothing" is deliberately **not** remembered: this node takes part in other coordinators' writes as a cohort member without passing through its own coordinator, and a commit acknowledged at super-majority reaches the remaining members in the background, so a remembered absence would hide a writer's own create behind a lagging member. Nothing weaker is remembered either — a silent peer, a claim, a thrown consult or an empty cohort lookup leaves no memo, so each such read consults again and keeps its `unavailable` flag (see § Unavailable reads). The memo is dropped when the block turns up locally, and whenever this node coordinates a pend or commit of the block, whatever the outcome. `'paranoid'` never uses it; `'off'` does, because the absence consult is what makes an absent answer authoritative at all, not stale-content repair. The cost, an accepted tradeoff stated with a `NOTE:` at the site: when the cohort-of-one view was transient (peers still mid-identify, see above), a block created through the grown cohort reads as absent for up to one window — the same one-window cost the held-block window pays for a cohort that grows.
