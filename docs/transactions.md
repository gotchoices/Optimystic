...
- **A claim needs a second, independent peer — unless the cohort has no second peer to give.** The corroboration floor is two distinct peers, capped by how many peers other than the reader could answer at all. That capacity is the **larger** of the peers currently visible and `assumedClusterSize - 1` — the operator's declaration of the smallest cohort this deployment can genuinely field, *not* `clusterSize` (the replication factor) — so a cohort view shrunk by a partition (or by an attacker with routing influence) cannot talk the requirement down to a single voter. Practical consequence, in both directions: **a genuine two-node deployment needs `assumedClusterSize: 2`** or its members cannot repair each other and every attempt logs `cluster-fetch:no-quorum` — `libp2p-node-base` already defaults it to `2`, so such a mesh works unconfigured; and **a large deployment must set `clusterPolicy.assumedClusterSize` to its real cohort size**, or that same default leaves its floor relaxable to a single voter by a shrunken view. Unlike the old advice of lowering `clusterSize`, neither setting shrinks the replication factor.
- **The same cap applies to the commit-path reconcile, on both of its gates.** `reconcile-block.ts` first picks the revision (as above) and then requires the cohort to agree on the block *content* at that revision. Both gates take the same capacity, so a two-node deployment at `assumedClusterSize: 2` (the `libp2p-node-base` default) can move bytes as well as pointers; set `assumedClusterSize` higher than the peers a two-node mesh can actually field and the heal declines with `reconcile:no-rev-quorum` or `reconcile:no-content-quorum`. Note what the content gate is and is not: block ids are random 256-bit strings, not content hashes, so a peer's bytes are checked against *other peers' bytes*, never against the id you asked for. With one other peer there is nothing to check against, and its content is accepted on the same trust its revision claim already receives.
