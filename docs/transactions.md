...
- **A claim needs a second, independent peer — unless the cohort has no second peer to give.** The corroboration floor is two distinct peers, capped by how many peers other than the reader could answer at all. That capacity is the **larger** of the peers currently visible and `assumedClusterSize - 1` — the operator's declaration of the smallest cohort this deployment can genuinely field, *not* `clusterSize` (the replication factor) — so a cohort view shrunk by a partition (or by an attacker with routing influence) cannot talk the requirement down to a single voter. Practical consequence: **a genuine two-node deployment must set `clusterPolicy.assumedClusterSize: 2`**, otherwise its members cannot repair each other and every attempt logs `cluster-fetch:no-quorum`. Unlike the old advice of lowering `clusterSize`, this does not also shrink the replication factor.
- **The same cap applies to the commit-path reconcile, on both of its gates.** `reconcile-block.ts` first picks the revision (as above) and then requires the cohort to agree on the block *content* at that revision. Both gates take the same capacity, so a two-node deployment that has declared `assumedClusterSize: 2` can move bytes as well as pointers; leave `assumedClusterSize` unset (it defaults to `minAbsoluteClusterSize`, 2) or set it too high in a two-node mesh and the heal declines with `reconcile:no-rev-quorum` or `reconcile:no-content-quorum`. Note what the content gate is and is not: block ids are random 256-bit strings, not content hashes, so a peer's bytes are checked against *other peers' bytes*, never against the id you asked for. With one other peer there is nothing to check against, and its content is accepted on the same trust its revision claim already receives.
