...
**Read the outcome logs, not the attempt.** `cluster-fetch:synced` is emitted only when the local `latest` actually advanced. A pass that selected a newer revision and still moved no data logs `cluster-fetch:not-restored` — now a genuine anomaly (the cohort corroborated a revision but could not supply agreeing content), not the expected steady state it used to be. If the block was missing locally, that outcome also stops the read from claiming the block is absent: the cohort just said it exists, so the entry is flagged `unavailable: 'peers-unreachable'` (see § Unavailable reads) instead. A declined or failed acquisition logs `cluster-fetch:acquire-error`; a forward revision that no local promotion could reach logs `cluster-fetch:promote-unavailable` and falls through to acquisition rather than failing the read. Either way the block is marked freshly checked, so a *stale* block's next repair attempt waits out the window — a block that is entirely missing locally is exempt from the window and retries on the very next read.
